...
public class Amr1RenameStrategy implements RenameStrategy {
    ...
    /// should be renamed to: 20211114-155745.outcoming.Iza Kapała.phone.+48 503 594 583.amr
    ...
    public boolean tryRename(Path value) {
        ...
        var timePart = fileName.substring(11, 19).replace("-", "").replace(":", ""); // hhmmss
        // Extract the rest of the filename
        var restPart = fileName.substring(19);
        // Create the new filename
        var newFileName = String.format("%s-%s.outcoming.%s", datePart, timePart, restPart);
